  ...
  it('Shows first image src is not blank and has correct caption and visible buttons', async () => {
    try {
      ...
      const [btnCloseDisplay, btnCloseOpacity,
        ...
        const { opacity: closeOpacity, display: closeDisplay } = window.getComputedStyle(modal.querySelector('.close'));
        const { opacity: leftOpacity, display: leftDisplay } = window.getComputedStyle(modal.querySelector('.left-arrow'));
        const { opacity: rightOpacity, display: rightDisplay } = window.getComputedStyle(modal.querySelector('.right-arrow'));
        return [
